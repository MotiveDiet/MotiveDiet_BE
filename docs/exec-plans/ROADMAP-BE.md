...
프론트엔드는 `./ROADMAP-FE.md` 참고. Phase 번호는 두 로드맵 간에 동일하게 맞춰져 있다. 실제 요청/응답 스키마는 `../design-docs/API.md` 참고 — 이 파일은 "무엇을 만드는지" 체크리스트이고, `../design-docs/API.md`가 "어떤 형태로 주고받는지" 명세다.
